...
def test_api_presence_and_types():
    ...
    for method in [
        ...
        "get_total_retransmissions",
    ]:
        assert hasattr(host, method), f"Missing method '{method}' in Host."
    assert isinstance(host.get_received_data(), str), (
        "get_received_data must return a string."
    )
    assert isinstance(host.get_current_window_size(), int), (
        "get_current_window_size must return an int."
    )
    assert isinstance(host.get_total_retransmissions(), int), (
        "get_total_retransmissions must return an int."
    )
...
def test_should_lose_hook():
    ...
    assert nic_b.received_chars == "ABDE", (
        "The should_lose hook failed to drop exactly payload 'C'."
    )
...
def test_router_forwarding():
    ...
    assert end_nic.received, (
        "Router failed to forward the packet to the outgoing interface."
    )
...
def test_queue_saturation():
    ...
    assert nic_out.queue_depth() <= 5, (
        "NIC queue exceeded its maximum defined capacity."
    )
...
def test_simple_topology_integration():
    ...
    host_a, host_b, _, _, _ = setup_topology(sim, host_b_class=AssertHost)
